...
#ifndef FACE_DEMO_FACEDETECTOR_H
#define FACE_DEMO_FACEDETECTOR_H
...
public:
    ...
    MTCNN(ModelReader::ModelLoader *modeloader);

    vector<FaceInfo>
    Detect_mtcnn(const cv::Mat &img, const int min_size, const float *threshold, const float factor, const int stage);
...
MTCNN::MTCNN(const string &proto_model_dir) {
    ...
    ONet_ = cv::dnn::readNetFromCaffe(proto_model_dir + "/det3-half.prototxt",
                                      proto_model_dir + "/det3-half.caffemodel");
}

MTCNN::MTCNN(ModelReader::ModelLoader *modeloader) {
    ModelReader::Model *modelDet1 = modeloader->readModel(0);
    ModelReader::Model *modelDet2 = modeloader->readModel(1);
    ModelReader::Model *modelDet3 = modeloader->readModel(2);
    PNet_ = cv::dnn::readNetFromCaffe(modelDet1->prototxtBuffer, modelDet1->modelsize.prototxt_size,
                                      modelDet1->caffemodelBuffer, modelDet1->modelsize.caffemodel_size);
    RNet_ = cv::dnn::readNetFromCaffe(modelDet2->prototxtBuffer, modelDet2->modelsize.prototxt_size,
                                      modelDet2->caffemodelBuffer, modelDet2->modelsize.caffemodel_size);
    ONet_ = cv::dnn::readNetFromCaffe(modelDet3->prototxtBuffer, modelDet3->modelsize.prototxt_size,
                                      modelDet3->caffemodelBuffer, modelDet3->modelsize.caffemodel_size);
}
...
//    cout << "feature_map_w_: " << feature_map_w_ << ", feature_map_h_: " << feature_map_h_ << endl;
...
//            cout << "i:" << i << ", x1: " << faceBox.xmin << ", y1: " << faceBox.ymin << endl;
//            cout << "i:" << i << ", x2: " << faceBox.xmax << ", y2: " << faceBox.ymax << endl;
...
//            cout << "i:" << i << ", x: " << faceInfo.bbox_reg[0] << ", y: " << faceInfo.bbox_reg[1] << endl;

            faceBox.score = confidence_data[i];
//            cout << "score: " << confidence_data[i] << endl;
            ...
            switch (methodType) {
                ...
                case 'm':
                    if (static_cast<float>(area_intersect) / std::min(area1, area2) > thresh){
                        ...
                        mask_merged[i] = 1;
                        cout << "area_intersect: " << area_intersect << endl;
                        cout << "std::min(area1, area2): " << std::min(area1, area2) << endl;
                        cout << "m: " << static_cast<float>(area_intersect) / std::min(area1, area2) << endl;
                    }
                    break;
                default:
                    break;
...
vector<FaceInfo>
MTCNN::NextStage(const cv::Mat &image, vector<FaceInfo> &pre_stage_res, int input_w, int input_h, int stage_num,
                 const float threshold) {
    vector<FaceInfo> res;
    ...
    switch (stage_num) {
        ...
        case 3: {
            ...
            const float *confidence_data_bak = (float *) confidence->data;
            for(int i = 0; i < inputs.size() ; i++){
                cout << confidence_data_bak[2*i+1] << endl;
                cout << inputs[i].cols << ", " << inputs[i].rows << endl;
                cv::imshow("s", inputs[i]);
                cv::waitKey(0);
                cv::imwrite("imgs/1.jpg", inputs[i]);
            }
...
vector<FaceInfo> MTCNN::ProposalNet(const cv::Mat &img, int minSize, float threshold, float factor) {
    ...
    for (int i = 0; i < scales.size(); i++) {
        ...
        cv::Mat inputBlob = cv::dnn::blobFromImage(resized, std_val, cv::Size(),
                                                   cv::Scalar(mean_val, mean_val, mean_val), false);
        // debug
//        cv::imshow("1 stage", resized);
//        cv::waitKey(0);
//        float *c = (float *)inputBlob.data;
        ...
        // debug
//        cout << "prob: " << prob.size() << endl;
//        cout << "reg: " << reg.size() << endl;
...
vector<FaceInfo>
MTCNN::Detect_mtcnn(const cv::Mat &image, const int minSize, const float *threshold, const float factor,
                    const int stage) {
...
#endif //FACE_DEMO_FACEDETECTOR_H
